...
---
---List of files/directories for doxygen to read
---@field files string[]
---
...
local function inline_type(obj, classes)
  ...
  if not cls.inlinedoc then
    ...
    local period = (obj.desc == '' or vim.endswith(obj.desc, '.')) and ''
      or '.'
  ...
  elseif desc == '' then
    ...
    elseif cls.parent then
      desc = desc
        .. fmt('Extends |%s| with the additional fields:', cls.parent)
    else
...
local function render_fields_or_params(
  xs,
  generics,
  classes,
  exclude_types,
  ...
  for _, p in ipairs(xs) do
    ...
    local desc = p.classvar
        and string.format('See |%s|.', cfg.fn_helptag_fmt(p))
      or p.desc

    local fnm = p.kind == 'operator' and fmt('op(%s)', nm)
      or fmt_field_name(nm)
    ...
    if ty then
      ...
      if desc then
        ...
        if #pty > TEXT_WIDTH - indent then
          ...
          table.insert(
            ret,
            md_to_vimdoc(desc, 9 + indent, 9 + indent, TEXT_WIDTH, true)
          )
        else
          desc = fmt('%s %s', pty, desc)
          table.insert(
            ret,
            md_to_vimdoc(desc, 1, 9 + indent, TEXT_WIDTH, true)
          )
        end
      else
...
local function render_class(class, classes, cfg)
  ...
  if class.desc then
    table.insert(
      ret,
      md_to_vimdoc(class.desc, INDENTATION, INDENTATION, TEXT_WIDTH)
    )
  end

  local fields_txt =
    render_fields_or_params(class.fields, nil, classes, nil, cfg)
...
local function render_fun_header(fun, cfg)
  ...
  local proto = fun.table and nm
    or nm .. '(' .. table.concat(args, ', ') .. ')'
...
local function render_returns(returns, generics, classes, exclude_types)
  ...
  for _, p in ipairs(returns) do
    ...
    table.insert(
      ret,
      md_to_vimdoc(table.concat(blk, ' '), 8, 8, TEXT_WIDTH, true)
    )
  end
...
local function render_fun(fun, classes, cfg)
  ...
  if fun.since then
    ...
    if since == 0 or (info.prerelease and since == info.level) then
      ...
      local v =
        assert(util.version_level[since], 'invalid @since on ' .. fun.name)
  ...
  if fun.desc then
    table.insert(
      ret,
      md_to_vimdoc(fun.desc, INDENTATION, INDENTATION, TEXT_WIDTH)
    )
  end
  ...
  if fun.notes then
    ...
    for _, p in ipairs(fun.notes) do
      table.insert(
        ret,
        '      • ' .. md_to_vimdoc(p.desc, 0, 8, TEXT_WIDTH, true)
      )
    end
  end
  ...
  if fun.params and #fun.params > 0 then
    local param_txt = render_fields_or_params(
      fun.params,
      fun.generics,
      classes,
      cfg.exclude_types,
  ...
  if fun.returns then
    local txt =
      render_returns(fun.returns, fun.generics, classes, cfg.exclude_types)
...
local function gen_target(cfg)
  ...
  if vim.uv.fs_stat(cfg.filename) then
    delete_lines_below(cfg.filename, first_section_tag)
  end

  local fp = assert(io.open(cfg.filename, 'a'))
  fp:write(table.concat(docs, '\n'))
  fp:close()
end

---@param config nvim.gen_vimdoc.Config
